...
> **Two known backend gaps (reported, not yet built — see Dev-Guide §11):** no crews API (in-app crew mgmt impossible; in-app-created TH has no crew till seeded) · no org-config update endpoint (Settings is read-only). **Sentry: deferred by user, do not raise.**
