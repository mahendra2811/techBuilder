...
> **RESEARCH-3 IS DONE.** A critical second-opinion review was run (Claude Fable 5 CLI, code-grounded) → `docs/techBuilder-SecondOpinion-Review.md`; Claude web turned it into two accepted docs at **[`docs/research/reserch-3/`](docs/research/reserch-3/)**: **`techBuilder-Hardening-Punchlist.md`** (AUTHORITATIVE pre-pilot work order) + **`techBuilder-Pilot-Playbook.md`** (pilot product spec).
> **NEXT SESSION STARTS AT: [`docs/research/reserch-3/techBuilder-Hardening-Punchlist.md`](docs/research/reserch-3/techBuilder-Hardening-Punchlist.md)** — execute work packages **top-to-bottom, tier by tier** (P0 trust/correctness: WP-1 RBAC scope enforcement → WP-2 self-approval guard → WP-3 edit/void ownership+window → WP-4 backdating policy → WP-5 money-math unit tests → WP-6 honest sync posture; then P1 pilot infra; then P2 pilot surface; then P3 doc reconciliation). Each WP has an Acceptance block — implement, then prove it. Where the punchlist conflicts with older docs, **punchlist + Build-Readiness Spec win**.
> Key review findings (full detail in the review doc): backend enforces only boolean `can()` — **scope (OWN_SITE/OWN_CREW/SELF) is enforced NOWHERE**; no self-approval check in approvals; no ownership/time-window on record update/void; sync `pull` is a stub + outbox unwired (Roadmap gate #2 oversells); zero tests on wage calc; Expo Go must NOT be the pilot channel (EAS APK only); no server-side backups; no bulk merchant seed.
> **Still-relevant context:** app RUNS on the phone via Expo Go (SDK 54) in MOCK mode; mock logins (any password): `acme_owner`, `sm1`, `th1`, `driver1`, `worker1`. The earlier **Milestone A plan** (`docs/techBuilder-NextSteps-and-LiveBackend-Plan.md`, flip to real backend over WiFi `192.168.31.15:4000`) is now largely **subsumed by the punchlist** (WP-7 hosted backend, WP-10 bulk seed) — reconcile with the user which path to run first when resuming.
