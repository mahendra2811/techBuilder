...
- **Backdated-creation policy (records — Phase 4, web pivot):** creating any businessDate-stamped record (progress/expense/fuel/vehicle-log/trip/material-txn/issue) obeys the same shape: **Team Head ≤2 days** back · **Site Manager ≤7 days** · **Driver ≤2 days** · future dates rejected · Owner unlimited (Owner can create only via override paths — `record.enter` is denied to Owner). Enforced on the REST creates AND the sync (outbox) CREATE path. Leave is deliberately unwindowed (planned absences are future-dated by nature). *Note: very-late idempotent create retries (same id, beyond the window) are rejected before the idempotency check — acceptable while clients are online-required.*
- **Password change:** the new password must differ from the current one (a forced change that keeps the same password defeats its purpose).
